...
class BoardParse
{
  ...
  function tweet($href)
  {
    ...
    if(in_array($host, $twitter_or_x_hostnames) && !empty($path))
    {
      // The tweet ID is the last part of the path, and it should be numeric.
      $tweet_id = end(explode('/', $path));
      if (!empty($tweet_id) && is_numeric($tweet_id))
      {
        // This is the target element. The actual embedding is done in
        // js/embedding.js.
        $embed = '<span class="embedded-tweet embedded" data-tweet-id="' . $tweet_id . '"></span>';
        // Alternate text for quoting (just the URL).
        $embed .= '<span class="embedded-url" style="display: none;">' . $href[1] . '</span>';
        return $embed;
      }
    }
    return $href[1];
  ...
  function tiktok($href)
  {
    $url = parse_url($href[1]);
    $host = isset($url['host']) ? $url['host'] : '';
    // Is the URL from an TikTok hostname with a path component?
    $tiktok_hostnames = ['tiktok.com', 'www.tiktok.com'];
    if(in_array($host, $tiktok_hostnames))
    {
      // This is the target element. The actual embedding is done in
      // js/embedding.js.
      $embed = '<blockquote class="tiktok-embed embedded" cite="' . $href[1] . '" muted style="max-width: 325px;min-width: 325px;margin-left: 0;margin-right: 0;"></blockquote>';
      // Alternate text for quoting (just the URL).
      $embed .= '<span class="embedded-url" style="display: none;">' . $href[1] . '</span>';
      return $embed;
    }
  ...
  function instagram($href)
  {
    ...
    $host = isset($url['host']) ? $url['host'] : '';
    $path = isset($url['path']) ? rtrim($url['path'], '/') : '';
    // Is the URL from an Instagram hostname with a path component?
    $instagram_hostnames = ['instagram.com', 'www.instagram.com'];
    if(in_array($host, $instagram_hostnames) && !empty($path))
    {
  ...
  function run($string)
  {
    ...
    if($this->hidemedia)
    {
      ...
      $s = preg_replace("#\[instagram\](.*)\[\/instagram\]#Ui","<a href=\"$1\" onclick=\"window.open(this.href); return false;\">INSTAGRAM REMOVED CLICK TO VIEW</a>",$s);
      $s = preg_replace("#\[tiktok\](.*)\[\/tiktok\]#Ui","<a href=\"$1\" onclick=\"window.open(this.href); return false;\">TIKTOK REMOVED CLICK TO VIEW</a>",$s);
    }
    else
    {
      ...
      $s = preg_replace_callback("#\[instagram\](.*)\[\/instagram\]#Ui",array(&$this,'instagram'),$s);
      $s = preg_replace_callback("#\[tiktok\](.*)\[\/tiktok\]#Ui",array(&$this,'tiktok'),$s);
    }
